...
function MyProfileEdit({ user, setUser }) {
  ...
  function handleSubmit(e) {
      ...
      fetch(`/users/${user.id}`, {
        ...
        body: JSON.stringify({ first_name, last_name, email, img, is_contributor, phone }),
      }).then((res) => res.json());
    }
  ...
  return (
    <div className="edit-profile segment off-white">
      <form onSubmit={handleSubmit}>
        ...
        <input type="text" placeholder="Image URL" value={img} onChange={(e) => setImg(e.target.value)}></input>
        <input type="text" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)}></input>
        <input type="text" placeholder="Phone" value={phone} onChange={(e) => setPhone(e.target.value)}></input>
        Contributor? <input type="checkbox" value={is_contributor} onChange={(e) => setIsContrib(e.target.value)}></input>
